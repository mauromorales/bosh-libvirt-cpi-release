...
EOL
    ...
    before(:each) do
      ...
      allow(vm).to receive(:attach_device)
      allow(volume).to receive(:path).and_return('/disk/path')
    end

    it 'attaches the disk with `disk_cid` to the vm with `vm_cid`' do
      subject.attach_disk('vm-cid', 'disk-cid')

      expect(vm).to have_received(:attach_device).with(xml)
    end
  end
end
